...
Env vars:
  ...
  EXTRA_PORTS     comma-separated TCP ports to also open as plain accept/
                   close stubs (e.g. "4840") — used by Module 3 Day 4 to
                   give one Control-zone device a port the firewall
                   ruleset's R02 (Supervisory->Control OPC-UA) can be
                   tested against, without implementing real OPC-UA here:
                   the firewall filters by port, not payload, so a stub
                   listener is enough to prove the rule allows/denies it.
"""
import os
import random
import socket
